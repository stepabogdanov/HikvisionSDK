...
BOOL CManageDlg::OnInitDialog()
{
	...
	//tab
	m_ctrlTab.InsertItem(0,"Log query");
	m_DlgLogSearch.Create(IDD_DIALOG_LOG_SEARCH,&m_ctrlTab);
	m_ctrlTab.InsertItem(1,"Device status");
...
void CManageDlg::OnButtonDefaultCfg() 
{
	...
	if(IDOK == MessageBox("Restore default config？","Warning",MB_OKCANCEL))
	{
        if(!NET_DVR_RestoreConfig(lLoginID))
		{
			MessageBox("Restore fail");
			return;
		}
	}
...
void CManageDlg::OnButtonReboot() 
{
	...
	if(IDOK == MessageBox("Reboot device？","Warning",MB_OKCANCEL))
	{
		if(!NET_DVR_RebootDVR(lLoginID))
		{
			MessageBox("Reboot fail");
		    return;
		}
	}
...
void CManageDlg::OnButtonShutdown() 
{
	...
	if(IDOK == MessageBox("Shut down device？","Warning",MB_OKCANCEL))
	{
		if(!NET_DVR_ShutDownDVR(lLoginID))
		{
			MessageBox("Shut down fail");
			return;
		}
	}
...
void CManageDlg::DoGetDeviceResoureCfg()
{
	...
	else        //
	{
		...
		{
			if(IpAccessCfg.struIPChanInfo[i].byEnable)  //
			{
