...
BOOL CDlgLogSearch::OnInitDialog() 
{
	...
	m_listDeviceLog.InsertColumn(0, "Index", LVCFMT_RIGHT, 60, -1);
	m_listDeviceLog.InsertColumn(1, "Time", LVCFMT_LEFT, 140, -1);
	m_listDeviceLog.InsertColumn(2, "Main Type", LVCFMT_LEFT,80, -1);
	m_listDeviceLog.InsertColumn(3, "Second Type", LVCFMT_LEFT, 150, -1);
	m_listDeviceLog.InsertColumn(4, "Remote Host", LVCFMT_LEFT, 100, -1);
	m_listDeviceLog.InsertColumn(5, "Describe", LVCFMT_LEFT, 251, -1);
...
void CDlgLogSearch::OnBtnSearchLog() 
{
	...
	if (!m_bSearch)
	{
		...
		if (m_hFileThread == NULL)
		{
			MessageBox("Open log search thread fail");
			return;
		}
		GetDlgItem(IDC_BTN_SEARCH)->SetWindowText("stop search");
		m_bSearch = TRUE;
	}
	else
	{
