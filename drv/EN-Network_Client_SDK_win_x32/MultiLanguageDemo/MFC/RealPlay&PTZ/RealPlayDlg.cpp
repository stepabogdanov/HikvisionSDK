...
void CRealPlayDlg::StartPlay(int iChanIndex)
{
	...
	if(-1 == m_lPlayHandle)
	{
		...
		CString m_csErr;
        m_csErr.Format("Play error,code:%d",err);
		MessageBox(m_csErr);
	}
...
void CRealPlayDlg::OnButtonRecord() 
{
	if(m_lPlayHandle == -1)
	{
        MessageBox("Please select one channel");
		return;
	}
	if(!m_bIsRecording)
	{
...
void CRealPlayDlg::StartRecord()
{
	...
	sprintf(RecName,"%04d%02d%02d%02d%02d%02d_ch%02d.mp4",CurTime.GetYear(),CurTime.GetMonth(),CurTime.GetDay(), \
     ...
     m_bIsRecording = TRUE;
	 GetDlgItem(IDC_BUTTON_RECORD)->SetWindowText("Stop record");
}
...
void CRealPlayDlg::OnButtonPlay() 
{
	...
	if(!m_bIsPlaying)
	{
		 ...
		 m_bIsPlaying = TRUE;
		 GetDlgItem(IDC_BUTTON_PLAY)->SetWindowText("Stop play");
...
void CRealPlayDlg::OnButtonPresetGoto() 
{
	...
	if(m_lPlayHandle >= 0)
	{
		...
		{
			MessageBox("Call preset point error");
			return;
		}
	}
	else
...
void CRealPlayDlg::OnButtonPresetSet() 
{
	...
	if(m_lPlayHandle >= 0)
	{
		...
		{
			MessageBox("Set preset point error");
			return;
		}
	}
	else
...
void CRealPlayDlg::OnButtonPresetDel() 
{
	...
	if(m_lPlayHandle >= 0)
	{
		...
		{
			MessageBox("Delete preset point error");
			return;
		}
	}
	else
...
void CRealPlayDlg::OnButtonSeqGoto() 
{
	...
	if(!m_bIsOnCruise)
	{
        ...
        m_bIsOnCruise = TRUE;
		GetDlgItem(IDC_BUTTON_SEQ_GOTO)->SetWindowText("Stop");
	}
	else
	{
        if(m_lPlayHandle >= 0)
		...
		m_bIsOnCruise = FALSE;
		GetDlgItem(IDC_BUTTON_SEQ_GOTO)->SetWindowText("Call");
	}
...
void CRealPlayDlg::OnButtonTrackRun() 
{
	if(!m_bTrackRun)
	{
		...
		m_bTrackRun = TRUE;
		GetDlgItem(IDC_BUTTON_TRACK_RUN)->SetWindowText("Stop");    
	}
	else
	{
        //随便发送一个云台控制命令停止运行
		...
		m_bTrackRun = FALSE;
		GetDlgItem(IDC_BUTTON_TRACK_RUN)->SetWindowText("Run");   
	}
...
void CRealPlayDlg::OnBtnPtzAuto() 
{
	...
	if(!m_bAutoOn)
	{
	   ...
	   else
	   {
        ...
        GetDlgItem(IDC_BTN_PTZ_AUTO)->SetWindowText("Stop");
		m_bAutoOn = TRUE;
	}
	else
	{
		 ...
		 GetDlgItem(IDC_BTN_PTZ_AUTO)->SetWindowText("Auto");
		 m_bAutoOn = FALSE;
	}
	}
...
void CRealPlayDlg::OnBtnAuxPwron() 
{
	if(m_lPlayHandle >= 0)
	{
		if(!m_bAuxOn1)
		{
			...
			GetDlgItem(IDC_BTN_AUX_PWRON)->SetWindowText("Stop1");
			m_bAuxOn1 = TRUE;
		}
		else
		{
			NET_DVR_PTZControl(m_lPlayHandle,AUX_PWRON1,1);
			GetDlgItem(IDC_BTN_AUX_PWRON)->SetWindowText("Aux1");
			m_bAuxOn1 = FALSE;
		}
	}
...
void CRealPlayDlg::OnBtnAuxPwron2() 
{
	if(m_lPlayHandle >= 0)
	{
		if(!m_bAuxOn2)
		{
			...
			GetDlgItem(IDC_BTN_AUX_PWRON2)->SetWindowText("Stop2");
			m_bAuxOn2 = TRUE;
		}
		else
		{
			NET_DVR_PTZControl(m_lPlayHandle,AUX_PWRON2,1);
			GetDlgItem(IDC_BTN_AUX_PWRON2)->SetWindowText("Aux2");
			m_bAuxOn2 = FALSE;
		}
	}
...
void CRealPlayDlg::OnBtnFanPwron() 
{
	if(m_lPlayHandle >= 0)
	{
		if(!m_bFanOn)
		{
			...
			GetDlgItem(IDC_BTN_FAN_PWRON)->SetWindowText("Stop");
			m_bFanOn = TRUE;
		}
		else
		{
			NET_DVR_PTZControl(m_lPlayHandle,FAN_PWRON,1);
			GetDlgItem(IDC_BTN_FAN_PWRON)->SetWindowText("Fan");
			m_bFanOn = FALSE;
		}
	}
...
void CRealPlayDlg::OnBtnHeaterPwron() 
{
	if(m_lPlayHandle >= 0)
	{
		if(!m_bHeaterOn)
		{
			...
			GetDlgItem(IDC_BTN_HEATER_PWRON)->SetWindowText("Stop");
			m_bHeaterOn = TRUE;
		}
		else
		{
			NET_DVR_PTZControl(m_lPlayHandle,HEATER_PWRON,1);
			GetDlgItem(IDC_BTN_HEATER_PWRON)->SetWindowText("Heat");
			m_bHeaterOn = FALSE;
		}
	}
...
void CRealPlayDlg::OnBtnLightPwron() 
{
	if(m_lPlayHandle >= 0)
	{
		if(!m_bLightOn)
		{
			...
			GetDlgItem(IDC_BTN_LIGHT_PWRON)->SetWindowText("Turn off");
			m_bLightOn = TRUE;
		}
		else
		{
			NET_DVR_PTZControl(m_lPlayHandle,LIGHT_PWRON,1);
			GetDlgItem(IDC_BTN_LIGHT_PWRON)->SetWindowText("Light");
			m_bLightOn = FALSE;
		}
	}
...
void CRealPlayDlg::OnBtnWiperPwron() 
{
	...
	{	
		if(!m_bWiperOn)
		{
		    ...
		    GetDlgItem(IDC_BTN_WIPER_PWRON)->SetWindowText("Stop");
		    m_bWiperOn = TRUE;
		}
	    else
		{
            NET_DVR_PTZControl(m_lPlayHandle,WIPER_PWRON,1);
	    	GetDlgItem(IDC_BTN_WIPER_PWRON)->SetWindowText("Wiper");
	    	m_bWiperOn = FALSE;
		}
