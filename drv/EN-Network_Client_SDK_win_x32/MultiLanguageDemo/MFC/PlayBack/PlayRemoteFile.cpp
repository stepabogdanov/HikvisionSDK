...
//	LONG hFindHandle = NET_DVR_FindFile_V30(g_struDevInfo[g_iCurDevIndex].lLoginID,&FileCond);
	...
	else
	{
		...
		pDlg->GetDlgItem(IDC_STATIC_SEARCH)->SetWindowText("Searching...");
		...
		while(ret>0)
		{
			...
			{
				pDlg->GetDlgItem(IDC_STATIC_SEARCH)->SetWindowText("Search exception!");
				pDlg->MessageBox("Search exception");
				break;
			}
			else if(NET_DVR_FILE_NOFIND == ret)
			{
			    pDlg->GetDlgItem(IDC_STATIC_SEARCH)->SetWindowText("No record file!");
				pDlg->MessageBox("No record file");
				break;
				
			}
			else if(NET_DVR_NOMOREFILE == ret)   //查找结束
			{
				pDlg->GetDlgItem(IDC_STATIC_SEARCH)->SetWindowText("Search finish!");
				break;
			}
			...
			else if(NET_DVR_FILE_SUCCESS == ret)  //获取文件信息成功
			{
				...
				Property.Format("%s",FindData.byLocked == 1 ? "Locked file" : "Normal file");
...
BOOL CPlayRemoteFile::OnInitDialog() 
{
 ...
 //   GetDlgItem(IDC_STATIC_SEARCH)->ShowWindow(SW_HIDE);
	//初始化文件列表
	m_ctrlFileList.InsertColumn(0,"File name",LVCFMT_LEFT,90);
	m_ctrlFileList.InsertColumn(1,"Size",LVCFMT_LEFT,50);
	m_ctrlFileList.InsertColumn(2,"Start time",LVCFMT_LEFT,85);
	m_ctrlFileList.InsertColumn(3,"End time",LVCFMT_LEFT,85);
	m_ctrlFileList.InsertColumn(4,"Property",LVCFMT_LEFT,50);
...
void CPlayRemoteFile::InsertFileList()
{
	...
	for(int i=0; i<FileNum; i++)
	{
        ...
        Property.Format("%s",FileInfo.byLocked == 1 ? "Locked file" : "Normal file");
...
BOOL CPlayRemoteFile::StartPlayback()
{
	...
	if(pos == NULL)
	{
		MessageBox("Please select one file");
		return FALSE;
	}
	if(FALSE == StopPlayback())
		return FALSE;
...
//	m_hPlayHandle = NET_DVR_PlayBackByName(g_struDevInfo[g_iCurDevIndex].lLoginID,pFileName,hPlayWnd);
	...
	if(-1 == m_hPlayHandle)
	{
		...
		CString tmp;
		tmp.Format("Playback fail，error code:%d",err);
		MessageBox(tmp);
		return FALSE;
	}
...
BOOL CPlayRemoteFile::StopPlayback()
{
    if(m_hPlayHandle >= 0)
	{
		 ...
		 if(!NET_DVR_StopPlayBack(m_hPlayHandle))
		 {
			 int err = NET_DVR_GetLastError();
			 CString tmp;
			 tmp.Format("Stop playback fail,error code:%d",err);
		     MessageBox(tmp);
			 return FALSE;
		 }
		 //状态变量设置
...
void CPlayRemoteFile::OnButtonDownload() 
{
    ...
    if(m_lDownloadHandle < 0)
	{
		...
		if (m_lDownloadHandle >= 0)
		{
			...
			GetDlgItem(IDC_BUTTON_DOWNLOAD)->SetWindowText("Stop");
		}
		else
		{
			MessageBox("Download file fail");
			return;
		}
	}
	else
...
void CPlayRemoteFile::OnReleasedcaptureSliderVolume(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	{
		MessageBox("Set audio volume fail!");
		return;
	}
	*pResult = 0;
}
...
void CPlayRemoteFile::OnButtonRemoteSave() 
{
	if(m_hPlayHandle < 0)
	{
		MessageBox("Please start playback first!");
		return;
	}
