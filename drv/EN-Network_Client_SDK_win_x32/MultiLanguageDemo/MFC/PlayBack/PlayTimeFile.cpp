...
void CPlayTimeFile::OnBtnSave() 
{
	...
	if(m_hPlayHandle == -1)
	{
		MessageBox("Please playback first");
		return;
	}
	...
	if(!m_bIsSaving)
	{
		...
		m_bIsSaving = TRUE;
		GetDlgItem(IDC_BUTTON_SAVE)->SetWindowText("Stop save");
	}
	else
	{
		NET_DVR_StopPlayBackSave(m_hPlayHandle);
		m_bIsSaving = FALSE;
		GetDlgItem(IDC_BUTTON_SAVE)->SetWindowText("Save");
	}
...
void CPlayTimeFile::OnBtnStopTime() 
{
	...
	if(m_hPlayHandle >= 0)
	{
		...
		m_IsPause = FALSE;
	    m_bChangeSpeed = FALSE;
		GetDlgItem(IDC_BUTTON_PLAY_BYTIME)->SetWindowText("Play");
		//停止保存
		NET_DVR_StopPlayBackSave(m_hPlayHandle);
		m_bIsSaving = FALSE;
		GetDlgItem(IDC_BUTTON_SAVE)->SetWindowText("Save");
