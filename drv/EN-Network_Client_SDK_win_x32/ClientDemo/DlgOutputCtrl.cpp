						  ...
						  DWORD dwPTZCommand, DWORD dwPresetIndex, char *szCmdString)
{
	...
	if (iDevIndex >= 0)
	{
		...
		lChanNum = g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].iChannelNO;
	...
	if (lRealHandle >= 0 && g_bInSide)
	{
		...
		{
			bRet = NET_DVR_PTZPreset_EX(lRealHandle, dwPTZCommand, dwPresetIndex);//on preview socket
		}
		else
		{
		...
		if (bRet)
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_SUCC_T, "NET_DVR_PTZPreset[%d_EX] %s[%d]presetindex[%d]", g_struLocalParam.bPreviewBlock, szCmdString, dwPTZCommand, dwPresetIndex);
		}
		else
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_FAIL_T, "NET_DVR_PTZPreset[%d_EX] %s[%d]presetindex[%d]", g_struLocalParam.bPreviewBlock, szCmdString,dwPTZCommand, dwPresetIndex);
		}
	}
	else if (lLoginID >= 0 && iChanIndex >= 0)
	{
		...
		if (bRet)
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_SUCC_T, "NET_DVR_PTZPreset_Other %s %s[%d]presetindex[%d]", g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName, szCmdString, dwPTZCommand, dwPresetIndex);
		}
		else
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_FAIL_T, "NET_DVR_PTZPreset_Other %s %s[%d]presetindex[%d]", g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName, szCmdString, dwPTZCommand, dwPresetIndex);
		}
	}
	return bRet;
}
...
BOOL PTZCruiseAll(LONG lRealHandle, int iDevIndex, int iChanIndex, DWORD dwPTZCruiseCmd,BYTE byCruiseRoute, BYTE byCruisePoint, WORD wInput, char *szCmdString)
{
	...
	if (lRealHandle >= 0 && g_bInSide)
	{
		if (g_struLocalParam.bPreviewBlock)
		{
			bRet = NET_DVR_PTZCruise_EX(lRealHandle, dwPTZCruiseCmd, byCruiseRoute, byCruisePoint, wInput);//on preview socket
		}
		else
		{
		...
		if (bRet)
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_SUCC_T, "NET_DVR_PTZCruise[%d_EX] %s cruise[%d] point[%d] input[%d]", g_struLocalParam.bPreviewBlock, szCmdString, byCruiseRoute, byCruisePoint, wInput);
		}
		else
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_FAIL_T, "NET_DVR_PTZCruise[%d_EX] %s cruise[%d] point[%d] input[%d]", g_struLocalParam.bPreviewBlock, szCmdString, byCruiseRoute, byCruisePoint, wInput);
		}	
	...
	else if (lLoginID >= 0 && iChanIndex >= 0)
	{
		...
		if (bRet)
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_SUCC_T, "NET_DVR_PTZCruise_Other %s cruise[%d] point[%d] input[%d] %s", szCmdString, byCruiseRoute, byCruisePoint, wInput, g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName);
		}
		else
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_FAIL_T, "NET_DVR_PTZCruise_Other %s cruise[%d] point[%d] input[%d] %s", szCmdString, byCruiseRoute, byCruisePoint, wInput, g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName);
		}
	}
	return bRet;
}
...
/*********************************************************
  ...
  Return:	none
**********************************************************/
BOOL PTZTrackAll(LONG lRealHandle, int iDevIndex, int iChanIndex, DWORD dwPTZCommand, char *szCmdString)
{
	BOOL bRet = FALSE;
	LONG lLoginID = -1;
	LONG lChanNum = -1;
	...
	if (lRealHandle >= 0 && g_bInSide)
	{
		if (g_struLocalParam.bPreviewBlock)
		{
			bRet = NET_DVR_PTZTrack_EX(lRealHandle, dwPTZCommand);//on preview socket
		}
		else
		{
		...
		if (bRet)
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_SUCC_T, "NET_DVR_PTZTrack[%d_EX] %s[%d]", g_struLocalParam.bPreviewBlock, szCmdString, dwPTZCommand);
		}
		else
		{
			g_dlgOutput[g_pMainDlg->m_iCurWndIndex].AddLog(OPERATION_FAIL_T, "NET_DVR_PTZTrack[%d_EX] %s[%d]", g_struLocalParam.bPreviewBlock, szCmdString, dwPTZCommand);
		}
	}
	else if (lLoginID >= 0 && iChanIndex >= 0)
	{
		...
		if (bRet)
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_SUCC_T, "NET_DVR_PTZTrack_Other %s %s[%d]", g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName, szCmdString, dwPTZCommand);
		}
		else
		{
			g_pMainDlg->AddLog(iDevIndex, OPERATION_FAIL_T, "NET_DVR_PTZTrack_Othe %s %s[%d]", g_struDeviceInfo[iDevIndex].struChanInfo[iChanIndex].chChanName, szCmdString, dwPTZCommand);
		}
	}
	return bRet;
}
...
void CDlgOutputCtrl::OnBnClickedBtnTrackStartMem()
{
	...
	int iChanIndex = g_pMainDlg->GetCurChanIndex();
	int iWndIndex = g_pMainDlg->m_iCurWndIndex;
	PTZTrackAll(g_dlgOutput[iWndIndex].m_lPlayHandle, iDevIndex, iChanIndex, STA_MEM_CRUISE, "STA_MEM_CRUISE");	
}
...
void CDlgOutputCtrl::OnBnClickedBtnTrackStopMem()
{
	...
	int iChanIndex = g_pMainDlg->GetCurChanIndex();
	int iWndIndex = g_pMainDlg->m_iCurWndIndex;
	PTZTrackAll(g_dlgOutput[iWndIndex].m_lPlayHandle, iDevIndex, iChanIndex, STO_MEM_CRUISE, "STO_MEM_CRUISE");	
}
...
void CDlgOutputCtrl::OnBnClickedBtnTrackRun()
{
	...
	int iChanIndex = g_pMainDlg->GetCurChanIndex();
	int iWndIndex = g_pMainDlg->m_iCurWndIndex;
	PTZTrackAll(g_dlgOutput[iWndIndex].m_lPlayHandle, iDevIndex, iChanIndex, RUN_CRUISE, "RUN_CRUISE");	
}
