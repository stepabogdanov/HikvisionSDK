...
BOOL CDlgRemoteHideAlarm::OnInitDialog() 
{
	...
	m_bChkEmapAlarmOut = (g_lpHideAlarm->strHideAlarmHandleType.dwHandleType>>6)&0x01;
	m_bChkPicFtp = (g_lpHideAlarm->strHideAlarmHandleType.dwHandleType>>9)&0x01;
	int i=0;
...
void CDlgRemoteHideAlarm::OnBnClickedBtnHaDayOk()
{
    ...
    {
        g_lpHideAlarm->struAlarmTime[i][0].byStartHour = (BYTE)m_iHour11;
	    g_lpHideAlarm->struAlarmTime[i][0].byStartMin = (BYTE)m_iMin11;
	    g_lpHideAlarm->struAlarmTime[i][0].byStopHour = (BYTE)m_iHour12;
	    g_lpHideAlarm->struAlarmTime[i][0].byStopMin = (BYTE)m_iMin12;
	    g_lpHideAlarm->struAlarmTime[i][1].byStartHour = (BYTE)m_iHour21;
	    g_lpHideAlarm->struAlarmTime[i][1].byStartMin = (BYTE)m_iMin21;
	    g_lpHideAlarm->struAlarmTime[i][1].byStopHour = (BYTE)m_iHour22;
	    g_lpHideAlarm->struAlarmTime[i][1].byStopMin = (BYTE)m_iMin22;
	    g_lpHideAlarm->struAlarmTime[i][2].byStartHour = (BYTE)m_iHour31;
	    g_lpHideAlarm->struAlarmTime[i][2].byStartMin = (BYTE)m_iMin31;
	    g_lpHideAlarm->struAlarmTime[i][2].byStopHour = (BYTE)m_iHour32;
	    g_lpHideAlarm->struAlarmTime[i][2].byStopMin = (BYTE)m_iMin32;
	    g_lpHideAlarm->struAlarmTime[i][3].byStartHour = (BYTE)m_iHour41;
	    g_lpHideAlarm->struAlarmTime[i][3].byStartMin = (BYTE)m_iMin41;
	    g_lpHideAlarm->struAlarmTime[i][3].byStopHour = (BYTE)m_iHour42;
	    g_lpHideAlarm->struAlarmTime[i][3].byStopMin = (BYTE)m_iMin42;
	    g_lpHideAlarm->struAlarmTime[i][4].byStartHour = (BYTE)m_iHour51;
	    g_lpHideAlarm->struAlarmTime[i][4].byStartMin = (BYTE)m_iMin51;
	    g_lpHideAlarm->struAlarmTime[i][4].byStopHour = (BYTE)m_iHour52;
	    g_lpHideAlarm->struAlarmTime[i][4].byStopMin = (BYTE)m_iMin52;
	    g_lpHideAlarm->struAlarmTime[i][5].byStartHour = (BYTE)m_iHour61;
	    g_lpHideAlarm->struAlarmTime[i][5].byStartMin = (BYTE)m_iMin61;
	    g_lpHideAlarm->struAlarmTime[i][5].byStopHour = (BYTE)m_iHour62;
	    g_lpHideAlarm->struAlarmTime[i][5].byStopMin = (BYTE)m_iMin62;
	    g_lpHideAlarm->struAlarmTime[i][6].byStartHour = (BYTE)m_iHour71;
	    g_lpHideAlarm->struAlarmTime[i][6].byStartMin = (BYTE)m_iMin71;
	    g_lpHideAlarm->struAlarmTime[i][6].byStopHour = (BYTE)m_iHour72;
	    g_lpHideAlarm->struAlarmTime[i][6].byStopMin = (BYTE)m_iMin72;
	    g_lpHideAlarm->struAlarmTime[i][7].byStartHour = (BYTE)m_iHour81;
	    g_lpHideAlarm->struAlarmTime[i][7].byStartMin = (BYTE)m_iMin81;
	    g_lpHideAlarm->struAlarmTime[i][7].byStopHour = (BYTE)m_iHour82;
	    g_lpHideAlarm->struAlarmTime[i][7].byStopMin = (BYTE)m_iMin82;
    }
...
void CDlgRemoteHideAlarm::OnBnClickedBtnHideAlarmOk()
{
	...
	g_lpHideAlarm->strHideAlarmHandleType.dwHandleType |= (m_bChkEmapAlarmOut<<6);
	g_lpHideAlarm->strHideAlarmHandleType.dwHandleType |= (m_bChkPicFtp << 9);
	for (i=0; i<MAX_ALARMOUT_V30; i++)
	{
...
void CDlgRemoteHideAlarm::OnBnClickedBtnHaDayCopy()
{
    ...
    else if (m_nCopyTime == 7)
    {
        for (k = 0; k < MAX_TIMESEGMENT_V30; k++)
        {
            memcpy(&m_struHideHolidayHandle.struAlarmTime[k], &g_lpHideAlarm->struAlarmTime[i][k], sizeof(NET_DVR_SCHEDTIME));
        }
        if (!SetHideHolidayHandle(m_struHideHolidayHandle))
        {
            AfxMessageBox("Fail to set holiday handle");
