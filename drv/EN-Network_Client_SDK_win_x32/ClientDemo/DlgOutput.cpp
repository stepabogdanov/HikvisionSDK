...
LONG CDlgOutput::StartPlayInsideDecode(PCHANNEL_INFO pChanInfo, HTREEITEM hChanItem)
{
	...
	if (g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].lRealHandle < 0)
	{
		...
		//	struCardPlayInfo.lChannel = g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].iChanIndex+g_struDeviceInfo[iDeviceIndex].iStartChan;
			struCardPlayInfo.lChannel = m_lChannel;
			struCardPlayInfo.lLinkMode = g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwProtocol;
...
//			struPlayInfo.sMultiCastIP = g_struDeviceInfo[iDeviceIndex].chDeviceMultiIP;
			struPlayInfo.dwLinkMode = g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwLinkMode;
			struPlayInfo.dwStreamType = g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwStreamType;

			//start soft decode
			struPlayInfo.byPreviewMode = g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwPreviewMode;
	...
	if (m_lPlayHandle >= 0)
	{
		g_pMainDlg->AddLog(iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_RealPlay_V40 %s stream[%d] pro[%d]",\
			g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName, \
			g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwProtocol>>31 &0x1, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwProtocol&0xff);
		...
		if (!NET_DVR_MakeKeyFrame(g_struDeviceInfo[iDeviceIndex].lLoginID,  g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].iChannelNO))    //重置I帧
		{
			g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_MakeKeyFrame %s", g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName);
		}	
		//set player buffer
 		if (!NET_DVR_SetPlayerBufNumber(m_lPlayHandle,m_iNetDelayBufNum))
 		{
			g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SetPlayerBufNumber[%d] %s",m_iNetDelayBufNum, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName);
 		}
		...
		if (g_struLocalParam.dwBFrameNum > 0)
		{
 			...
 			{
				g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_ThrowBFrame[%d] %s",g_struLocalParam.dwBFrameNum, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName);
 			}
		}
		...
		g_struDeviceInfo[pChanInfo->iDeviceIndex].struChanInfo[pChanInfo->iChanIndex].lRealHandle = m_lPlayHandle;
	...
	else
	{
 		g_pMainDlg->AddLog(pChanInfo->iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_RealPlay_V40 chanindex[%d] stream[%d] pro[%d]", \
 			iChanIndex, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwProtocol>>31 &0x1, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwProtocol&0xff);
...
LONG CDlgOutput::StartPlay(PCHANNEL_INFO pChanInfo, HTREEITEM hChanItem)
{
	...
	//	struPlayInfo.sMultiCastIP = g_struDeviceInfo[iDeviceIndex].chDeviceMultiIP;
		...
		if (g_bStandardCB) //call back standard stream or not
		{
			...
			if (m_lPlayHandle >= 0)
			{
				...
				{
					g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SetStandardDataCallBack %s", g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName);
				}
			}
		}
		else
	...
	if (m_lPlayHandle >= 0)
	{
		...
		m_hChanItem = hChanItem;
		memcpy(&m_struChanInfo, pChanInfo, sizeof(CHANNEL_INFO));
		g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].lRealHandle = m_lPlayHandle;
	...
	{
 		g_pMainDlg->AddLog(pChanInfo->iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_RealPlay_V40 chanindex[%d] stream[%d] pro[%d]", \
 			iChanIndex, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].dwStreamType, g_struDeviceInfo[iDeviceIndex].struZeroChan[iChanIndex].dwLinkMode);
...
BOOL CDlgOutput::StopPlay(void)
{
	...
	{
		g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].lRealHandle = -1;
	}
...
void CDlgOutput::StartLocalRecord()
{
    ...
    {
		g_struDeviceInfo[m_struChanInfo.iDeviceIndex].struChanInfo[m_struChanInfo.iChanIndex].bLocalManualRec = TRUE;
    }
...
void CDlgOutput::StopLocalRecord()
{
    ...
    {
		g_struDeviceInfo[m_struChanInfo.iDeviceIndex].struChanInfo[m_struChanInfo.iChanIndex].bLocalManualRec = FALSE;
    }
	::PostMessage(g_pMainDlg->m_hWnd, WM_CHANGE_CHANNEL_ITEM_IMAGE, WPARAM(m_struChanInfo.iDeviceIndex), LPARAM(m_struChanInfo.iChanIndex));
}
...
void CDlgOutput::AddLog(int iLogType, const char* format, ...)
{
	...
	{
		g_pMainDlg->AddLog(iDeviceIndex, iLogType, "%s %s winindex[%d] ", szLogInfo, g_struDeviceInfo[iDeviceIndex].struChanInfo[iChanIndex].chChanName, m_iSubWndIndex);
	}
}
