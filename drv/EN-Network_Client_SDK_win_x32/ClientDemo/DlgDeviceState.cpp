...
// 		}
		...
		//由于struWorkState.struChanStatic[i]是按64个通道排列，而g_struDeviceInfo[m_iDeviceIndex].struChanInfo[i]是按48个紧凑排列
		//所以需要转换一下，i按紧凑排列
		int iChannelIndex = g_struDeviceInfo[m_iDeviceIndex].struChanInfo[i].iChannelNO-1;
		
		strChanNum.Format("%s", g_struDeviceInfo[m_iDeviceIndex].struChanInfo[i].chChanName);	
		
		if (0 == struWorkState.struChanStatic[iChannelIndex].byRecordStatic)
		{
			g_StringLanType(szLan, "不录像", "Not Record");
			m_csRecState.Format(szLan);
		}
		else if (1 == struWorkState.struChanStatic[iChannelIndex].byRecordStatic)
		{
			g_StringLanType(szLan, "录像", "Record");
			m_csRecState.Format(szLan);
		}
		
		if (0 == struWorkState.struChanStatic[iChannelIndex].bySignalStatic)
		{
			g_StringLanType(szLan, "正常", "Normal");
			m_csSignalState.Format(szLan);
		}
		else if (1 == struWorkState.struChanStatic[iChannelIndex].bySignalStatic)
		{
			g_StringLanType(szLan, "信号丢失", "Signal loss");
			m_csSignalState.Format(szLan);
		}
		
		if ( 0 == struWorkState.struChanStatic[iChannelIndex].byHardwareStatic)
		{
			g_StringLanType(szLan, "正常", "Normal");
			m_csHardWareState.Format(szLan);
		}
		else if (1 == struWorkState.struChanStatic[iChannelIndex].byHardwareStatic)
		{
			g_StringLanType(szLan, "异常", "Abnormal");
			m_csHardWareState.Format(szLan);
		}
		
		m_csLinkCount.Format("%d", struWorkState.struChanStatic[iChannelIndex].dwLinkNum);
		iTotalLen+=struWorkState.struChanStatic[iChannelIndex].dwLinkNum;
		m_csBitrate.Format("%ld", struWorkState.struChanStatic[iChannelIndex].dwBitRate);
		//strTemp.Format("%d", );
		...
		m_listChanState.SetItemText(iIndex, 6, struWorkState.struChanStatic[iChannelIndex].struClientIP[0].sIpV4);
		strTemp.Format("%d", struWorkState.struChanStatic[iChannelIndex].dwIPLinkNum);
