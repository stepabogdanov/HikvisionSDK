...
BOOL CDlgVcaRuleCfg::InitRuleType(int iBehaviorAbilityType)
{
    ...
    if ((iBehaviorAbilityType & REACH_HIGHT_ABILITY) != 0)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_REACH_HIGHT);
        index++;
    }
	...
	if (iBehaviorAbilityType & TRAIL_ABILITY)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_TRAIL);
        index++;
    }
	...
	if (iBehaviorAbilityType & FALL_DOWN_ABILITY)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_FALL_DOWN);
        index++;
    }
	...
	if (iBehaviorAbilityType & AUDIO_ABNORMAL_ABILITY)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_AUDIO_ABNORMAL);
        index++;
    }
    ...
    if (iBehaviorAbilityType & OVER_TIME_ABILITY)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_OVER_TIME);
        index++;
    }
    ...
    if (iBehaviorAbilityType & HUMAN_ENTER_ABILITY)
    {
        ...
        m_comboRuleType.InsertString(index, szLan);
        m_comboRuleType.SetItemData(index, VCA_HUMAN_ENTER);
        index++;
    }
...
Return:	 BOOL 
**********************************************************/
BOOL CDlgVcaRuleCfg::ShowRuleTypeWnd(VCA_EVENT_TYPE struEventType)
{
    ...
    if (struEventType == VCA_TRAVERSE_PLANE || struEventType == VCA_REACH_HIGHT)
    {
        ...
        else if (struEventType == VCA_REACH_HIGHT)
        {
            GetDlgItem(IDC_CHK_DRAW_LINE)->ShowWindow(SW_SHOW);
    ...
    else
    {
        ...
        else if (struEventType == VCA_VIOLENT_MOTION)
        {
            g_StringLanType(szLan, "触发时间阈值:", "Trigger time:");
            m_staticAlarmText.SetWindowText(szLan);
        ...
        else if (struEventType == VCA_GET_UP)
        {
            g_StringLanType(szLan, "触发时间阈值:", "Trigger time:");
            m_staticAlarmText.SetWindowText(szLan);
            m_staticAlarmArea.SetWindowText("1-100s");
		...
		else if (struEventType == VCA_TRAIL)
        {
            GetDlgItem(IDC_STATIC_SENSITIVE)->ShowWindow(SW_SHOW);
            GetDlgItem(IDC_COMBO_SENSITIVE)->ShowWindow(SW_SHOW);
			GetDlgItem(IDC_EDIT_ALARM_PERSIST)->ShowWindow(SW_HIDE);
		...
		else if (struEventType == VCA_FALL_DOWN)
        {
			g_StringLanType(szLan, "触发时间阈值:", "Trigger time:");
            m_staticAlarmText.SetWindowText(szLan);
		...
		else if (struEventType == VCA_AUDIO_ABNORMAL)
        {
        ...
        else if (struEventType == VCA_OVER_TIME)
        {
            g_StringLanType(szLan, "触发时间阈值:", "Trigger time:");
            m_staticAlarmText.SetWindowText(szLan);
            m_staticAlarmArea.SetWindowText("");
        ...
        else if (struEventType == VCA_HUMAN_ENTER)
        {
            GetDlgItem(IDC_EDIT_LEAVE_DELAY)->ShowWindow(SW_HIDE);
			GetDlgItem(IDC_EDIT_STATIC_DELAY)->ShowWindow(SW_HIDE);
			GetDlgItem(IDC_STATIC_LEAVE_DELAY)->ShowWindow(SW_HIDE);
		...
		else if (struEventType == VCA_LEAVE_POSITION)
		{
			GetDlgItem(IDC_EDIT_ALARM_PERSIST)->ShowWindow(SW_HIDE); 
            GetDlgItem(IDC_STATIC_SENSITIVE)->ShowWindow(SW_HIDE);
...
void CDlgVcaRuleCfg::OnSelchangeComboRuleType() 
{
    // TODO: Add your control notification handler code here
    UpdateData(TRUE);
    VCA_EVENT_TYPE eEventType;
    
    eEventType = (VCA_EVENT_TYPE)GetEventTypeFromWnd();
    ShowRuleTypeWnd(eEventType);
    if (eEventType == m_struRuleCfg.struRule[m_iCurRuleID].dwEventType)
    {
    ...
    m_struOneRule.dwEventType = eEventType; 
    UpdateData(FALSE);
}
...
void CDlgVcaRuleCfg::F_DrawFun(long lPlayHandle, HDC hDc, DWORD dwUser)
{
    ...
    if(	m_struOneRule.dwEventType== VCA_ENTER_AREA
        ...
        || m_struOneRule.dwEventType == VCA_OVER_TIME
		...
		|| m_struOneRule.dwEventType == VCA_TRAIL
		|| m_struOneRule.dwEventType == VCA_FALL_DOWN
        )
    {
        F_DrawFrame(hDc);
    }
    else if (m_struOneRule.dwEventType == VCA_TRAVERSE_PLANE
        || m_struOneRule.dwEventType == VCA_REACH_HIGHT)
    {
        F_DrawLine(hDc);
    }
...
void CDlgVcaRuleCfg::F_DrawLine(HDC hDc)
{
    ...
    CDrawFun::DrawVcaLine(hDc, &struTraversePlane);
}
...
void CDlgVcaRuleCfg::SetVcaRuleToWnd(NET_VCA_RULECFG_V41 const &struVcaRuleCfg)
{
    ...
    m_comboPicQuality.SetCurSel(m_struRuleCfg.struPictureParam.wPicQuality);
...
void CDlgVcaRuleCfg::SetOneVcaRuleToWnd(NET_VCA_ONE_RULE_V41 const &struVcaOneRule)
{
    ...
    SetEventTypeToWnd(struVcaOneRule.dwEventType);
    SetEventUnionToWnd(struVcaOneRule.uEventParam, struVcaOneRule.dwEventType);
...
{   
    ...
    struOneRule.dwEventType = (VCA_EVENT_TYPE)GetEventTypeFromWnd();
    GetEventUnionFromWnd(struOneRule.uEventParam, struOneRule.dwEventType);
...
void CDlgVcaRuleCfg::GetRelRecordChanFromWnd(BYTE  *byRelRecordChan, int nChanNum)
{
    ...
    {
        byRelRecordChan[i] = (BYTE)m_listRecordChan.GetCheck(i);
    }
}
