...
BOOL CDlgVoiceTalk::OnInitDialog()
{
	...
	char sTemp[20] = {0};
	for(int i = 1; i <= g_struDeviceInfo[m_dwDeviceIndex].iAudioNum+64; i ++)
	{
		...
		{
			sprintf(sTemp, "DigitalAudio%d", i-g_struDeviceInfo[m_dwDeviceIndex].iAudioNum+32);
 			m_comboVoiceChan.AddString(sTemp);
		}
	}
...
void CDlgVoiceTalk::OnBtnStart() 
{
	...
	if (m_dwSel < g_struDeviceInfo[m_dwDeviceIndex].iAudioNum) // 模拟对讲通道
	{
		...
		g_lVoiceHandle = NET_DVR_StartVoiceCom_V30(g_struDeviceInfo[m_dwDeviceIndex].lLoginID, m_dwSel+1, m_comboDataType.GetCurSel(), fAudioTalkDataCallBack, /*NULL,*/ NULL);//fVoiceDataCallBack
		if (g_lVoiceHandle == -1)
		{
	...
	{
		m_dwSel = m_dwSel - g_struDeviceInfo[m_dwDeviceIndex].iAudioNum + 32;
		g_lVoiceHandle = NET_DVR_StartVoiceCom_V30(g_struDeviceInfo[m_dwDeviceIndex].lLoginID, m_dwSel+1, m_comboDataType.GetCurSel(), fAudioTalkDataCallBack, /*NULL,*/ NULL);//fVoiceDataCallBack
		if (g_lVoiceHandle == -1)
		{
			g_pMainDlg->AddLog(m_dwDeviceIndex, OPERATION_FAIL_T, "NET_DVR_StartVoiceCom_V30 digital audio index[%d]", m_dwSel+1);
			g_StringLanType(szLan, "数字语音对讲失败", "Digital voice talk failed");
			AfxMessageBox(szLan);	
		}
		else
		{
			g_pMainDlg->AddLog(m_dwDeviceIndex, OPERATION_SUCC_T, "NET_DVR_StartVoiceCom_V30 digital audio[%d]", m_dwSel+1);
		}
	}
	EnableCtrl();
}
