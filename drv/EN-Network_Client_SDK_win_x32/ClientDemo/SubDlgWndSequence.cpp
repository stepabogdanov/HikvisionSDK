...
void CSubDlgWndSequence::OnInitialize(int nChanNum, int nIPChanNum, int nStartNum)
{
    ...
    for (i = 0; i < nIPChanNum; ++i)
    {
        ...
        if (g_struDeviceInfo[iDevIndex].struIPParaCfgV40.struStreamMode[i].uGetStream.struChanInfo.byIPID != 0 \
            && g_struDeviceInfo[iDevIndex].struIPParaCfgV40.struStreamMode[i].uGetStream.struChanInfo.byEnable == TRUE)
        {
