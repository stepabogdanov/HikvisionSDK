...
BOOL CDlgAudioUploadDownload::OnInitDialog() 
{
	...
	m_csDownloadPath.Format("%s", "C:\\Hiktest.wav");
	...
	m_fileDown.Open("D:\\Hiktest.wav", CFile::modeRead|CFile::modeWrite|CFile::modeNoTruncate|CFile::modeCreate);
...
void CDlgAudioUploadDownload::OnBtnUploadStart() 
{
	...
	else if (m_radioUpload == 1)
	{
		...
		fileTest.Close();
		//固定将文件“c:\Hiktest.wav”的数据读入到缓冲区，然后发送给设备（这个只是做测试用）
